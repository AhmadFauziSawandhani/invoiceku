...
<body>
    <div class="w-full" style="padding:5px;border: 1px solid #000;">
        ...
        <div class="w-full" style="margin:0;">
            <table autosize="1" style="border-collapse: collapse;font-size: 15pt; {{count($images) > 0 ? '' : 'width: 100%;'}}">
                <thead>
                    <tr style="font-weight: bold;">
                        <th class="border-td text-md col-height-md" style="width: 4%;">NO</th>
                        ...
                        <th class="border-td text-md" style="width: 10%;">PICKUP/Dooring</th>
                        <th class="border-td text-md" style="width: 10%;">REPACKING</th>
                        <th class="border-td text-md" style="width: 10%;">INSURANCE</th>
                        <th class="border-td text-md" style="width: 10%;">MINIMUM HDL</th>
                        <th class="border-td text-md" style="width: 10%;">SHIPDEX</th>
                        <th class="border-td text-md" style="width: 10%;">DUS UN</th>
                        <th class="border-td text-md" style="width: 10%;">ACC XRAY</th>
                        <th class="border-td text-md" style="width: 10%;">ADM SMU</th>
                ...
                <tbody style="font-size: 15pt;">
                    @foreach ($details as $item)
                        <tr>
                            <td class="text-center text-md border-left" style="width: 4%;">{{ $loop->iteration }}</td>
                            <td class="text-center text-md" style="width: 10%;">
                                {{ (new \App\Helpers\GeneralHelpers())->date_format_dmY($data->invoice_date) }}</td>
                            <td class="text-center text-md" style="width: 12%;">{{ $item->moda }}</td>
                            ...
                            <td class="text-center text-md" style="width: 8%;">{{ $item->colly }}</td>
                            ...
                            <td class="text-center text-md" style="width: 10%;">
                                {{ (new \App\Helpers\GeneralHelpers())->raw_currency($item->price_addons_pickup ? $item->price_addons_pickup : $item->price_addons_dooring) }}</td>
                            <td class="text-center text-md" style="width: 10%;">
                                {{ (new \App\Helpers\GeneralHelpers())->raw_currency($item->price_addons_packing) }}</td>
                            <td class="text-center text-md" style="width: 10%;">
                                {{ (new \App\Helpers\GeneralHelpers())->raw_currency($item->price_addons_insurance) }}</td>
                            <td class="text-center text-md" style="width: 10%;">
                                {{ (new \App\Helpers\GeneralHelpers())->raw_currency($item->minimum_hdl) }}</td>
                            <td class="text-center text-md" style="width: 10%;">
                                {{ (new \App\Helpers\GeneralHelpers())->raw_currency($item->shipdex) }}</td>
                            <td class="text-center text-md" style="width: 10%;">
                                {{ (new \App\Helpers\GeneralHelpers())->raw_currency($item->dus_un) }}</td>
                            <td class="text-center text-md" style="width: 10%;">
                                {{ (new \App\Helpers\GeneralHelpers())->raw_currency($item->acc_xray) }}</td>
                            <td class="text-center text-md" style="width: 10%;">
                                {{ (new \App\Helpers\GeneralHelpers())->raw_currency($item->adm_smu) }}</td>
                            <td class="text-center text-md" style="width: 10%;">
                    ...
                    @if ($row <= 5) {
                        <tr>
                            <td colspan='18' class='border-left border-right' style='line-height: 10px;'>
                                &nbsp;
                            </td>
                        </tr>
                    @endif
                    @if(count($details) > 0 && $details[0]->manifest)
                        <tr>
                            <td colspan='18'>
                                ...
                                @if($item->manifest)
                                    <tr>
                                        <th colspan="12" style="font-size: 8pt;background-color: yellow;">
                ...
                <tfoot style="width: 100%">
                    <tr style="font-size: 9pt;">
                        <td class="border-td" colspan="5" style="font-size:9pt;line-height: 20px;text-align: center;font-weight:bold;"><b>TOTAL</b>
                        </td>
                        <td class="border-td text-center" style="font-size:9pt;">{{ isset($totalColly) ? $totalColly : 0 }}</td>
                        <td class="border-td text-center" style="font-size:9pt;">
                            {{ isset($totalChargeableWeight) ? $totalChargeableWeight : 0 }}</td>
                        <td class="border-td" colspan="3" style="font-size:9pt;text-align: center;font-weight:bold;">
                            <b>SUBTOTAL</b>
                        </td>
                        <td class="border-td" colspan="8" style="font-size:9pt;font-weight:bold;text-align: right;">Rp.
                    ...
                    <tr>
                        ...
                        <td></td>
                        <td class="border-td" colspan="3" style="font-size:9pt;line-height: 20px;text-align: center;font-weight:bold;">PPN 1.1%
                        </td>
                        <td class="border-td" colspan="8" style="font-size:9pt;font-weight:bold;text-align: right;">Rp.
                            {{ (new \App\Helpers\GeneralHelpers())->raw_currency($data->ppn) }}</td>
                    </tr>
                    <tr>
                        <td class="border-td" colspan="2" style="font-size:9pt;line-height: 20px;width: 15%;"> INVOICE NO </td>
                        <td class="border-td" colspan="3" style="font-size:9pt;"><b>{{ $data->invoice_number }}</b></td>
                        <td></td>
                        <td></td>
                        <td class="border-td" colspan="3" style="font-size:9pt;text-align: center;font-weight:bold;">DOWN PAYMENT</td>
                        <td class="border-td" colspan="8" style="font-size:9pt;font-weight:bold;text-align: right;">Rp. {{ (new \App\Helpers\GeneralHelpers())->raw_currency($data->down_payment) }}</td>
                    </tr>
                    <tr>
                        <td class="border-td" colspan="2" style="font-size:9pt;line-height: 20px;width: 15%;"> DUE DATE </td>
                        <td class="border-td" colspan="3" style="font-size:9pt;line-height: 20px;width: 15%;"><b>
                                @php
                                    if ($data->payment_type == 2) {
                                        echo (new \App\Helpers\GeneralHelpers())->date_format_inv($data->payment_due_date);
                                    }
                                @endphp</b>
                        </td>
                        <td></td>
                        <td></td>
                        <td class="border-td" colspan="3" style="font-size:9pt;text-align: center;font-weight:bold;">TOTAL</td>
                        <td class="border-td" colspan="8" style="font-size:9pt;font-weight:bold;text-align: right;">
                            {{ (new \App\Helpers\GeneralHelpers())->raw_currency($data->total) }}</td>
                    </tr>
                    <tr>
                        <td class="border-td" colspan="2" style="font-size:9pt;line-height: 20px;width: 15%;"> IDR</td>
                        <td class="border-td" colspan="3" style="font-size:9pt;">
                            <b>{{ (new \App\Helpers\GeneralHelpers())->raw_currency($data->total) }}</b>
                        </td>
                        <td colspan="9"></td>
                    </tr>
                    <tr>
                        <td colspan="15" style="font-size:9pt;line-height: 20px;">&nbsp;</td>
                    </tr>
                    <tr>
                        <td class="border-td" colspan="2" style="font-size:9pt;line-height: 20px;width: 15%;"> Inword</td>
                        <td class="border-td" colspan="3" style="font-size:9pt;">
