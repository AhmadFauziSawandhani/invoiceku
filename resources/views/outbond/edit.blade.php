            ...
            methods: {
                ...
                onSelectChange(e, items, index) {
                    ...
                    if (isChecked) {
                        ...
                        this.invForm.total_colly += items.total_colly
                        this.invForm.total_weight += items.total_weight
                        this.invForm.total_volume += items.total_volume
                        this.invForm.total_volume_m3 += items.total_volume_m3
                    } else {
                ...
                deleteEmployee(item, index) {
                    const lastManifest = this.manifests.find(val => val.id == item.id)
                    if(lastManifest) this.manifests.find(val => val.id == item.id).selected = false
                    this.invForm.manifest.splice(index, 1)
                },
                onSubmitForm() {
                    ...
                    this.invForm.outbond_id = this.outbond.id
                    axios.post('{{ route('outbond.update') }}', {
                            ...this.invForm
                        })
                        .then((response) => {
